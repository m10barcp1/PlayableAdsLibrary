    ...
    execute: function () {
      ...
      _export("RoadSpline", RoadSpline = (_dec6 = ccclass('RoadSpline'), _dec7 = executeInEditMode(true), _dec8 = requireComponent(Graphics), _dec9 = menu('Spline/RoadSpline'), _dec10 = property({
        ...
        getPoint(u, out) {
          ...
          if (i >= n) i = n - 1;
          const t = g - i;
          const s = segs[i]; // inline cubic bezier (position)
        ...
        _resetAllToSmooth() {
          for (const knot of this.knots) knot.mode = (_crd && TangentMode === void 0 ? (_reportPossibleCrUseOfTangentMode({
            error: Error()
          }), TangentMode) : TangentMode).AutoSmooth;
        ...
        _draw() {
          ...
          if (this.knotRadius > 0) {
            for (let i = 0; i < n; i++) {
              const c = children[i].position; // first knot green, last knot red, others white

              if (i === 0) g.fillColor = new Color(60, 220, 90, 255);else if (i === n - 1 && !this.closed) g.fillColor = new Color(230, 70, 70, 255);else g.fillColor = new Color(255, 255, 255, 255);
...
//# sourceMappingURL=a3898a007ad3f82d82a02142da57c24cb533a3b0.js.map