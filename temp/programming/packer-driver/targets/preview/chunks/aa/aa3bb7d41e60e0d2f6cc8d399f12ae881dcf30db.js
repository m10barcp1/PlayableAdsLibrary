    ...
    execute: function () {
      ...
      _export("GameBootstrap", GameBootstrap = (_dec = ccclass('GameBootstrap'), _dec(_class = class GameBootstrap extends Component {
        start() {
          ...
          var scene = this.node.scene; // 1) Use the pre-baked world that lives inside the scene file.
          ...
          var world = this.findInScene(scene, 'World');
          ...
          var playerParts = (_crd && buildHumanoid === void 0 ? (_reportPossibleCrUseOfbuildHumanoid({
            error: Error()
          ...
          var sword = (_crd && createPrimitiveNode === void 0 ? (_reportPossibleCrUseOfcreatePrimitiveNode({
            error: Error()
          ...
          var stats = playerParts.root.addComponent(_crd && PlayerStats === void 0 ? (_reportPossibleCrUseOfPlayerStats({
            error: Error()
          }), PlayerStats) : PlayerStats);
          var inventory = playerParts.root.addComponent(_crd && Inventory === void 0 ? (_reportPossibleCrUseOfInventory({
            error: Error()
          }), Inventory) : Inventory);
          inventory.stats = stats;
          var controller = playerParts.root.addComponent(_crd && PlayerController === void 0 ? (_reportPossibleCrUseOfPlayerController({
            error: Error()
          ...
          var combat = playerParts.root.addComponent(_crd && PlayerCombat === void 0 ? (_reportPossibleCrUseOfPlayerCombat({
            error: Error()
          ...
          var camNode = scene.getChildByName('Main Camera');
          ...
          var cam = (_camNode$getComponent = camNode.getComponent(Camera)) != null ? _camNode$getComponent : camNode.addComponent(Camera);
          cam.clearColor = {
            x: 0.45,
            y: 0.55,
            z: 0.75,
          ...
          cam.fov = 55;
          cam.far = 500;
          var camFollow = camNode.getComponent(_crd && CameraFollow === void 0 ? (_reportPossibleCrUseOfCameraFollow({
            error: Error()
          ...
          var npcsParent = new Node('NPCs');
          world.addChild(npcsParent);

          for (var def of _crd && NPCDefs === void 0 ? (_reportPossibleCrUseOfNPCDefs({
            error: Error()
          }), NPCDefs) : NPCDefs) {
            var built = (_crd && buildNpcNode === void 0 ? (_reportPossibleCrUseOfbuildNpcNode({
              error: Error()
            }), buildNpcNode) : buildNpcNode)(def);
            npcsParent.addChild(built.root);
            var comp = built.root.addComponent(_crd && NPC === void 0 ? (_reportPossibleCrUseOfNPC({
              error: Error()
            }), NPC) : NPC);
          ...
          this.schedule(() => {
            var anyClose = false;
            var pp = playerParts.root.worldPosition;

            for (var c of npcsParent.children) {
              var a = c.worldPosition;
              var d = Math.hypot(a.x - pp.x, a.z - pp.z);
          ...
          var questNode = new Node('QuestManager');
          scene.addChild(questNode);
          var questMgr = questNode.addComponent(_crd && QuestManager === void 0 ? (_reportPossibleCrUseOfQuestManager({
            error: Error()
          }), QuestManager) : QuestManager);
          questMgr.inventory = inventory;
          questMgr.stats = stats; // 6) Enemy spawner

          var enemiesParent = new Node('Enemies');
          world.addChild(enemiesParent);
          var spawnerNode = new Node('EnemySpawner');
          scene.addChild(spawnerNode);
          var spawner = spawnerNode.addComponent(_crd && EnemySpawner === void 0 ? (_reportPossibleCrUseOfEnemySpawner({
            error: Error()
          ...
          var uiNode = new Node('UI');
          scene.addChild(uiNode);
          var ui = uiNode.addComponent(_crd && UIManager === void 0 ? (_reportPossibleCrUseOfUIManager({
            error: Error()
          }), UIManager) : UIManager);
          ui.stats = stats;
        ...
        findInScene(root, name) {
          ...
          for (var child of root.children) {
            var hit = this.findInScene(child, name);
            if (hit) return hit;
          }
