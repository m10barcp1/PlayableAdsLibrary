    ...
    execute: function () {
      ...
      _export("EnemySpawner", EnemySpawner = (_dec = ccclass('EnemySpawner'), _dec(_class = class EnemySpawner extends Component {
        ...
        spawnAll() {
          ...
          for (var zone of this.zones) {
            for (var i = 0; i < zone.count; i++) {
              var kind = zone.kinds[Math.floor(Math.random() * zone.kinds.length)];
              var ang = Math.random() * Math.PI * 2;
              var r = Math.random() * zone.radius;
              var x = zone.center.x + Math.cos(ang) * r;
              var z = zone.center.z + Math.sin(ang) * r;
        ...
        spawn(kind, pos) {
          ...
          var arch = (_crd && EnemyArchetypes === void 0 ? (_reportPossibleCrUseOfEnemyArchetypes({
            error: Error()
          }), EnemyArchetypes) : EnemyArchetypes)[kind];
          if (!arch) return null;
          var node = this.buildVisual(kind, arch);
          node.setPosition(pos);
          this.worldRoot.addChild(node);
          var en = node.addComponent(_crd && Enemy === void 0 ? (_reportPossibleCrUseOfEnemy({
            error: Error()
          }), Enemy) : Enemy);
          en.init(arch, this.player, this.playerStats, this.inventory); // Find body parts for animation

          var lArm = (_node$getChildByName = node.getChildByName('LeftArm')) != null ? _node$getChildByName : undefined;
          var rArm = (_node$getChildByName2 = node.getChildByName('RightArm')) != null ? _node$getChildByName2 : undefined;
          var lLeg = (_node$getChildByName3 = node.getChildByName('LeftLeg')) != null ? _node$getChildByName3 : undefined;
          var rLeg = (_node$getChildByName4 = node.getChildByName('RightLeg')) != null ? _node$getChildByName4 : undefined;
          var body = (_node$getChildByName5 = node.getChildByName('Body')) != null ? _node$getChildByName5 : undefined;
        ...
        buildVisual(kind, arch) {
          if (kind === 'wolf') {
            ...
            root.addChild(head);
            var tail = (_crd && createPrimitiveNode === void 0 ? (_reportPossibleCrUseOfcreatePrimitiveNode({
              error: Error()
          ...
          if (kind === 'boss') {
            // crown
            var crown = (_crd && createPrimitiveNode === void 0 ? (_reportPossibleCrUseOfcreatePrimitiveNode({
              error: Error()
