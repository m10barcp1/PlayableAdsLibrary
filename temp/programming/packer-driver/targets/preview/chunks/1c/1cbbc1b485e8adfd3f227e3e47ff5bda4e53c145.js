  ...
  return {
    ...
    execute: function () {
      ...
      _export("Enemy", Enemy = (_dec = ccclass('Enemy'), _dec(_class = class Enemy extends Component {
        ...
        takeDamage(dmg, from) {
          if (this.dead) return;
          var real = Math.max(1, Math.floor(dmg - this.archetype.defense * 0.5));
          this.hp -= real;
          this.hurtTimer = 0.15;
        ...
        die() {
          ...
          if (this.playerInventory) {
            for (var drop of this.archetype.drops) {
              if (Math.random() < drop.chance) {
                var _drop$min, _drop$max;

                var min = (_drop$min = drop.min) != null ? _drop$min : 1,
        ...
        update(dt) {
          ...
          var pp = this.player.worldPosition;
          var mp = this.node.worldPosition;
          var dx = pp.x - mp.x,
              dz = pp.z - mp.z;
          var dist = Math.hypot(dx, dz); // Apply knockback

          if (this.knockback.lengthSqr() > 0.01) {
            var np = this.node.position;
          ...
          if (dist <= this.archetype.attackRange) {
            ...
            var nx = dx / dist,
                nz = dz / dist;
            var _np = this.node.position;
            var sp = this.archetype.speed * dt;
            this.node.setPosition(_np.x + nx * sp, _np.y, _np.z + nz * sp); // face player
        ...
        animate() {
          ...
          var moving = this.state === 'chase';
          var swing = moving ? Math.sin(this.animTime * 10) * 30 : 0;
          ...
          if (this.state === 'attack' && this.bodyParts.r) {
            var a = Math.sin(this.animTime * 14) * 50 - 30;
