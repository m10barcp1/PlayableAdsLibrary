System.register(["cc"], function (_export, _context) {
  ...
  function buildMaterial(color) {
    ...
    var stdAsset = EffectAsset.get('builtin-standard');

    if (stdAsset) {
      var m = tryBuildMat(stdAsset, 'albedo', color);
      if (m) return m;
    } // Strategy 2 – extract effect from the engine's own default-material


    var baseMat = builtinResMgr.get('default-material');

    if (baseMat != null && baseMat.effectAsset) {
      var _m = tryBuildMat(baseMat.effectAsset, 'albedo', color);
    ...
    {
      var _m2 = tryBuildMat('builtin-standard', 'albedo', color);
    ...
    var unlitAsset = EffectAsset.get('builtin-unlit');

    if (unlitAsset) {
      var _m3 = tryBuildMat(unlitAsset, 'mainColor', color);
    ...
    {
      var _m4 = tryBuildMat('builtin-unlit', 'mainColor', color);
  ...
  function makeColorMaterial(color) {
    var key = color.r + "_" + color.g + "_" + color.b + "_" + color.a; // isValid catches destroyed materials; passes.length > 0 catches materials whose
    ...
    var cached = matCache.get(key);
    if (cached && cached.isValid && cached.passes.length > 0) return cached;
    var mat = buildMaterial(color); // Cache only custom materials that have valid passes.
    ...
    var defaultMat = builtinResMgr.get('default-material');
  ...
  function createPrimitiveNode(name, kind, size, color) {
    ...
    var mesh = utils.MeshUtils.createMesh(geom); // IMPORTANT: assign material BEFORE mesh.
