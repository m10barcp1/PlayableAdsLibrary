    ...
    execute: function () {
      ...
      _export("QuestManager", QuestManager = (_dec = ccclass('QuestManager'), _dec(_class = class QuestManager extends Component {
        ...
        onEnemyDied(enemy) {
          ...
          var kind = enemy == null || (_enemy$archetype = enemy.archetype) == null ? void 0 : _enemy$archetype.kind;
          if (!kind) return;
        ...
        checkReady(q) {
          var all = q.objectives.every(o => this.isObjectiveDone(q, o));
        ...
        canAccept(q) {
          ...
          if (q.prerequisite) {
            var pre = this.quests.find(x => x.id === q.prerequisite);
        ...
        accept(qid) {
          var q = this.quests.find(x => x.id === qid);
        ...
        turnIn(qid) {
          ...
          if (q.reward.items) for (var r of q.reward.items) this.inventory.addItem(r.id, r.count);
          q.state = 'completed';
          (_crd && EventBus === void 0 ? (_reportPossibleCrUseOfEventBus({
            error: Error()
          }), EventBus) : EventBus).emit((_crd && GameEvents === void 0 ? (_reportPossibleCrUseOfGameEvents({
        ...
        questsForNpc(npcId) {
          var available = this.quests.filter(q => q.giverNpcId === npcId && this.canAccept(q));
          var active = this.quests.filter(q => q.giverNpcId === npcId && q.state === 'active');
          var ready = this.quests.filter(q => q.giverNpcId === npcId && q.state === 'ready');
          return {
            available,
            active,
            ready
