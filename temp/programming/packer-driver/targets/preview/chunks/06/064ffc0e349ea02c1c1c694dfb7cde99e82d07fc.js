    ...
    execute: function () {
      ...
      _export("Inventory", Inventory = (_dec = ccclass('Inventory'), _dec(_class = class Inventory extends Component {
        ...
        addItem(itemId, count) {
          ...
          if (def.stackable) {
            var existing = this.slots.find(s => s.itemId === itemId);
        ...
        removeItem(itemId, count) {
          ...
          var idx = this.slots.findIndex(s => s.itemId === itemId);
        ...
        hasItem(itemId, count) {
          ...
          var s = this.slots.find(s => s.itemId === itemId);
        ...
        equip(itemId) {
          ...
          if (!this.hasItem(itemId)) return false;
          var slot = def.slot; // swap

          var prev = this.equipment[slot];
        ...
        recalcBonuses() {
          ...
          for (var slot of Object.keys(this.equipment)) {
            var _s$attack, _s$defense, _s$hp, _s$mp;

            var id = this.equipment[slot];
            if (!id) continue;
            var it = (_crd && ItemDB === void 0 ? (_reportPossibleCrUseOfItemDB({
              error: Error()
            }), ItemDB) : ItemDB)[id];
            var s = it.stats;
            if (!s) continue;
