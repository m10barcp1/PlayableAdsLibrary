    ...
    execute: function () {
      ...
      _export("SolitaireGame", SolitaireGame = (_dec = ccclass('SolitaireGame'), _dec(_class = class SolitaireGame extends Component {
        ...
        onLoad() {
          ...
          var canvasTr = (_this$node$parent = this.node.parent) == null ? void 0 : _this$node$parent.getComponent(UITransform);
          var w = canvasTr ? canvasTr.width : view.getVisibleSize().width;
          var h = canvasTr ? canvasTr.height : view.getVisibleSize().height;
          this.playfieldW = w;
        ...
        buildBackground() {
          ...
          var tr = bg.addComponent(UITransform);
          tr.setContentSize(this.playfieldW, this.playfieldH);
          var g = bg.addComponent(Graphics); // Radial-ish gradient simulated with two filled rects
          ...
          var innerW = this.playfieldW * 0.96;
          var innerH = this.playfieldH * 0.94;
        ...
        buildPlayfield() {
          ...
          var totalW = 7 * CARD_W + 6 * PILE_GAP_X;
          var startX = -totalW / 2 + CARD_W / 2; // Use top of playfield area for stock/waste/foundations.

          var topY = this.playfieldH / 2 - CARD_H / 2 - 90;
          var tabY = topY - CARD_H - 30; // Stock (column 0), Waste (column 1), Foundations (columns 3..6)
          ...
          for (var i = 0; i < 4; i++) {
            var slot = this.makeSlot("Foundation" + i, SUIT_GLYPH[i], startX + (3 + i) * (CARD_W + PILE_GAP_X), topY);
            this.foundationSlots.push(slot);
          } // Tableau slots


          for (var _i = 0; _i < 7; _i++) {
            var _slot = this.makeSlot("Tableau" + _i, '', startX + _i * (CARD_W + PILE_GAP_X), tabY);

            this.tableauSlots.push(_slot);
        ...
        buildHUD() {
          ...
          var headerY = this.playfieldH / 2 - 40;
        ...
        makeLabel(text, x, y, size, parent, color) {
          if (size === void 0) {
            size = 24;
          }
        ...
        buildButtons() {
          var bottomY = -this.playfieldH / 2 + 50;
          var total = 5;
          var spacing = 150;
        ...
        makeButton(text, x, y, w, h, color, cb, parent) {
          ...
          draw(color);
          var lbl = this.makeLabel(text, 0, 0, 22, n, new Color(255, 255, 255, 255));
        ...
        buildTutorialOverlay() {
          ...
          var tr = this.tutorialOverlay.addComponent(UITransform);
          tr.setContentSize(this.playfieldW, this.playfieldH);
          var dim = this.tutorialOverlay.addComponent(Graphics);
          dim.fillColor = new Color(0, 0, 0, 200);
          dim.rect(-this.playfieldW / 2, -this.playfieldH / 2, this.playfieldW, this.playfieldH);
          dim.fill();
          var panelW = Math.min(720, this.playfieldW - 80);
          var panelH = Math.min(560, this.playfieldH - 120);
          var panel = new Node('Panel');
          panel.layer = Layers.Enum.UI_2D;
          panel.addComponent(UITransform).setContentSize(panelW, panelH);
          var pg = panel.addComponent(Graphics);
        ...
        buildWinOverlay() {
          ...
          var tr = this.winOverlay.addComponent(UITransform);
          tr.setContentSize(this.playfieldW, this.playfieldH);
          var dim = this.winOverlay.addComponent(Graphics);
          dim.fillColor = new Color(0, 0, 0, 200);
          dim.rect(-this.playfieldW / 2, -this.playfieldH / 2, this.playfieldW, this.playfieldH);
          dim.fill();
          var panelW = 520,
          ...
          var winScore = this.makeLabel('Score: 0', 0, 50, 26, panel);
          var winMoves = this.makeLabel('Moves: 0', 0, 10, 26, panel);
          var winTime = this.makeLabel('Time: 0:00', 0, -30, 26, panel);
        ...
        newGame() {
          // Clear previous
          for (var c of this.allCards) if (c.node && c.node.isValid) c.node.destroy();
          ...
          while (idx < deck.length) {
            var _card = deck[idx++];
            _card.faceUp = false;
            _card.pileType = 'stock';
            _card.pileIndex = 0;
            _card.posInPile = this.stock.length;
            this.stock.push(_card);
          } // Position everything
        ...
        createCard(suit, rank) {
          var node = new Node("Card_" + RANK_GLYPH[rank] + SUIT_GLYPH[suit]);
          node.layer = Layers.Enum.UI_2D;
          var tr = node.addComponent(UITransform);
          tr.setContentSize(CARD_W, CARD_H);
          ...
          var cl = center.addComponent(Label);
          cl.fontSize = 36;
          cl.isBold = true;
        ...
        drawCard(card) {
          var g = card.node.getComponent(Graphics);
          g.clear();
          ...
          if (card.faceUp) {
            ...
            for (var yy = -CARD_H / 2 + 16; yy < CARD_H / 2 - 8; yy += 16) {
              for (var xx = -CARD_W / 2 + 16; xx < CARD_W / 2 - 8; xx += 16) {
                g.moveTo(xx, yy);
                g.lineTo(xx + 6, yy + 6);
                g.lineTo(xx + 12, yy);
                g.lineTo(xx + 6, yy - 6);
          ...
          var tl = card.node.getChildByName('tl').getComponent(Label);
          var br = card.node.getChildByName('br').getComponent(Label);
          var center = card.node.getChildByName('center').getComponent(Label);
          var col = isRed(card.suit) ? COLOR_RED : COLOR_BLACK;
          var txt = card.faceUp ? RANK_GLYPH[card.rank] + "\n" + SUIT_GLYPH[card.suit] : '';
        ...
        layoutAll(instant) {
          ...
          for (var i = 0; i < 4; i++) pilesToLay.push({
            type: 'foundation',
            index: i,
            arr: this.foundations[i]
          });

          for (var _i3 = 0; _i3 < 7; _i3++) pilesToLay.push({
            type: 'tableau',
            index: _i3,
            arr: this.tableau[_i3]
          });

          var siblingOrder = 0;

          for (var p of pilesToLay) {
            for (var _i4 = 0; _i4 < p.arr.length; _i4++) {
              var c = p.arr[_i4];
              c.pileType = p.type;
              c.pileIndex = p.index;
              c.posInPile = _i4;
              var target = this.cardPosInPile(p.type, p.index, _i4);
        ...
        onCardTouchStart(card, e) {
          ...
          var pile = this.getPile(card.pileType, card.pileIndex);
          var idx = pile.indexOf(card);
          ...
          var worldPos = e.getUILocation();
          var local = this.cardLayer.getComponent(UITransform).convertToNodeSpaceAR(new Vec3(worldPos.x, worldPos.y, 0));
          this.dragOffset.set(card.node.position.x - local.x, card.node.position.y - local.y); // Bring dragged group to front

          for (var c of this.dragging) c.node.setSiblingIndex(99999);
        }
        ...
        onCardTouchMove(card, e) {
          ...
          var worldPos = e.getUILocation();
          var local = this.cardLayer.getComponent(UITransform).convertToNodeSpaceAR(new Vec3(worldPos.x, worldPos.y, 0));
          var head = this.dragging[0];
          var newX = local.x + this.dragOffset.x;
          var newY = local.y + this.dragOffset.y;
          var dx = newX - head.node.position.x;
          var dy = newY - head.node.position.y;

          for (var c of this.dragging) {
            var p = c.node.position;
        ...
        onCardTouchEnd(card, e) {
          ...
          if (!this.dragging.length) {
            ...
            if (!card.faceUp && card.pileType === 'tableau') {
              var pile = this.tableau[card.pileIndex];

              if (pile[pile.length - 1] === card) {
                var wasFace = [card.faceUp];
                card.faceUp = true;
                this.history.push({
                  cards: [card],
                  fromType: 'tableau',
          ...
          var dropTarget = this.findDropTarget();
        ...
        findDropTarget() {
          var head = this.dragging[0];
          var center = head.node.position;
          var candidates = [];

          for (var i = 0; i < 4; i++) candidates.push({
            type: 'foundation',
            index: i,
            node: this.foundationSlots[i]
          });

          for (var _i5 = 0; _i5 < 7; _i5++) candidates.push({
            type: 'tableau',
            index: _i5,
            node: this.tableauSlots[_i5]
          });
          ...
          for (var c of candidates) {
            ...
            var pile = this.getPile(c.type, c.index);
            var pos = void 0;
            if (pile.length === 0) pos = c.node.getPosition();else pos = pile[pile.length - 1].node.getPosition();
            var dx = pos.x - center.x;
            var dy = pos.y - center.y;
            var d = Math.sqrt(dx * dx + dy * dy); // Must overlap reasonably with card area
        ...
        canPlace(cards, toType, toIndex) {
          ...
          if (toType === 'foundation') {
            ...
            var pile = this.foundations[toIndex];
          ...
          if (toType === 'tableau') {
            var _pile = this.tableau[toIndex];
            if (_pile.length === 0) return head.rank === 13;
            var _top = _pile[_pile.length - 1];
            if (!_top.faceUp) return false;
            return isRed(_top.suit) !== isRed(head.suit) && head.rank === _top.rank - 1;
          }

          return false;
        }

        executeMove(cards, fromType, fromIndex, toType, toIndex, recordHistory) {
          if (recordHistory === void 0) {
            recordHistory = true;
          }

          var fromPile = this.getPile(fromType, fromIndex);
          var toPile = this.getPile(toType, toIndex); // Remove from source

          var startIdx = fromPile.indexOf(cards[0]);
          fromPile.splice(startIdx, cards.length); // Add to destination

          for (var c of cards) toPile.push(c); // Determine flip of newly-exposed card
          ...
          if (fromType === 'tableau' && fromPile.length > 0) {
            var top = fromPile[fromPile.length - 1];
        ...
        undo() {
          ...
          if (m.flippedCardId !== undefined && (m.fromType !== m.toType || m.fromIndex !== m.toIndex || m.cards.length !== 1 || m.cards[0].id !== m.flippedCardId)) {
            ...
            var fl = this.allCards.find(c => c.id === m.flippedCardId);
          ...
          if (m.stockRecycled) {
            ...
            // Move cards back
            var toPile = this.getPile(m.toType, m.toIndex); // Remove the last m.cards.length cards from destination

            var moved = toPile.splice(toPile.length - m.cards.length, m.cards.length);
            var fromPile = this.getPile(m.fromType, m.fromIndex);
        ...
        hint() {
          ...
          for (var pile of this.tableau) {
            for (var c of pile) if (c.faceUp) sources.push(c);
          } // To foundation first


          for (var _c2 of sources) {
            var _pile2 = this.getPile(_c2.pileType, _c2.pileIndex);

            if (_pile2[_pile2.length - 1] !== _c2) continue;

            for (var i = 0; i < 4; i++) if (this.canPlace([_c2], 'foundation', i)) {
              return this.flashHint(_c2.node, this.foundationSlots[i]);
            }
          } // To tableau


          for (var _c3 of sources) {
            var _pile3 = this.getPile(_c3.pileType, _c3.pileIndex);

            var idx = _pile3.indexOf(_c3);

            var seq = _c3.pileType === 'tableau' ? _pile3.slice(idx) : [_c3];

            for (var _i6 = 0; _i6 < 7; _i6++) {
              if (_c3.pileType === 'tableau' && _c3.pileIndex === _i6) continue;

              if (this.canPlace(seq, 'tableau', _i6)) {
                var tgtPile = this.tableau[_i6];
                var target = tgtPile.length ? tgtPile[tgtPile.length - 1].node : this.tableauSlots[_i6];
                return this.flashHint(_c3.node, target);
              }
            }
        ...
        checkWin() {
          var total = this.foundations.reduce((s, p) => s + p.length, 0);

          if (total === 52) {
            this.running = false; // Bonus from time

            var timeBonus = Math.max(0, 700000 / Math.max(30, this.elapsed) | 0);
            this.score += timeBonus;
            this.refreshHUD();
            var w = this.winOverlay;
        ...
        tryAutoFinish() {
          ...
          var anyDown = this.tableau.some(p => p.some(c => !c.faceUp));
        ...
        celebrate() {
          ...
          for (var pile of this.foundations) {
            for (var c of pile) {
