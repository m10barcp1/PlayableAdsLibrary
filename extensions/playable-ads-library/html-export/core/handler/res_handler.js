...
class res_handler {
    ...
    create_html() {
        ...
        //设置title + favicon
        s_html_content = this._set_head(s_html_content);
        config_1.default.d_hot.s_base_html_content = s_html_content;
    }
    _get_title(s_html_content) {
        const $ = cheerio.load(s_html_content);
        var title = $('title').text();
        // bỏ tiền tố mặc định để lấy tên build (idempotent với cả prefix mới)
        title = title.replace("Cocos Creator | ", "");
        title = title.replace(config_1.default.html_title_prefix + " | ", "");
        return title || "";
    }
    // đặt lại title trang + nhúng favicon vào <head>
    _set_head(s_html_content) {
        const $ = cheerio.load(s_html_content);
        // title: "<prefix> | <tên build>"
        const prefix = config_1.default.html_title_prefix || "";
        const name = config_1.default.d_hot.s_title || "";
        const s_new_title = (prefix && name) ? `${prefix} | ${name}` : (prefix || name);
        if ($('title').length) {
            $('title').text(s_new_title);
        }
        else {
            $('head').append(`<title>${s_new_title}</title>`);
        }
        // favicon: gỡ link icon gốc (nếu có). Việc nhúng favicon ĐÃ MÃ HÓA được
        // chèn vào GIỮA file (trong body, xem channel_handler._get_middle_inject_script)
        // để không lộ ảnh logo dạng data:image plaintext ở <head>.
