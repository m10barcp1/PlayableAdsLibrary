...
const landscape_1 = __importDefault(require("../common/landscape"));
...
class channel_handler {
    async run() {
        ...
        // code bảo mật (favicon + logo) sẽ được chèn vào GIỮA khối tài nguyên
        const s_middle_inject = this._get_middle_inject_script();
        // #### 资源
        //导入不带压缩的资源 (kênh không nén: tách __res ra ~giữa để chèn vào)
        const s_res_body = this._split_res_with_inject(d_hot.d_res_cache, s_middle_inject);
        // #### 带压缩的资源
        let s_zip_res_body = "";
        {
            //导入带压缩的zip资源
            ...
            if (d_hot.s_out_res_zip_base64) {
                ...
                // chèn code bảo mật vào GIỮA: sau khối __zip, trước __res (~38% file)
                s_zip_res_body += s_middle_inject;
                //导入常规资源
                s_zip_res_body += `window.__res=${JSON.stringify(d_hot.d_out_res_no_zip)};`;
            }
            else {
                //không có khối nén -> tách __res ra ~giữa rồi chèn code bảo mật vào
                s_zip_res_body += this._split_res_with_inject(d_hot.d_out_res_no_zip, s_middle_inject);
            }
        }
        ...
        for (let key in inject_channel_adapter) {
            ...
            if (d_channel.b_html_compression) {
                ...
                if (d_channel.b_split_res) {
                    // tài nguyên tách ra res.js -> body nhỏ; vẫn chèn code bảo mật vào giữa
                    l_body = [s_channel_body, s_middle_inject, s_common_body];
                }
                else {
    ...
    _get_common_script(s_channel_name) {
        ...
        // script layout ngang + logo 2 bên và favicon được chèn vào GIỮA file
        // (xem _get_middle_inject_script), không còn để ở cuối body nữa.
        return s_base + s_pre_load_script + s_version_adapter_body + s_common;
    }
    /**
     * Gom code BẢO MẬT (favicon + logo landscape) thành JS thuần để chèn vào GIỮA
     * file - vùi giữa dữ liệu base64 tài nguyên, khó tìm hơn so với để ở đầu/cuối.
     */
    _get_middle_inject_script() {
        let s = "";
        const s_fav = landscape_1.default.get_favicon_inject_code(config_1.default.favicon);
        if (s_fav) {
            s += s_fav;
        }
        const s_land = landscape_1.default.get_inject_script(config_1.default.landscape);
        if (s_land) {
            s += s_land;
        }
        return s;
    ...
    /**
     * Phát ra window.__res nhưng TÁCH ĐÔI theo dung lượng để chèn s_inject vào ~giữa
     * (dùng cho kênh không nén - khi không có khối __zip làm ranh giới).
     */
    _split_res_with_inject(d_res, s_inject) {
        const keys = Object.keys(d_res || {});
        if (!s_inject) {
            return `window.__res=${JSON.stringify(d_res)};`;
        }
        if (keys.length < 2) {
            return `window.__res=${JSON.stringify(d_res)};` + s_inject;
        }
        let total = 0;
        ...
        if (cut <= 0 || cut >= keys.length) {
            return `window.__res=${JSON.stringify(d_res)};` + s_inject;
        }
        const a = {}, b = {};
        keys.forEach((k, i) => { (i < cut ? a : b)[k] = d_res[k]; });
        return `window.__res=${JSON.stringify(a)};` + s_inject + `Object.assign(window.__res,${JSON.stringify(b)});`;
    }
    //获得渠道脚本 
