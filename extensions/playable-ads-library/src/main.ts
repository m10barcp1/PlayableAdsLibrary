...
const DEFAULT_SYNC_URL = 'http://127.0.0.1:4650';
...
// ---- Library sync (server riêng) helpers ----------------------------------
// Resolve the sync server URL + token: prefer values the panel passes in, then
// the saved project profile, then the compile-time default.
async function resolveSyncArgs(opts: any): Promise<{ serverUrl: string; token: string; mirror: boolean }> {
    let serverUrl = opts && opts.serverUrl;
    let token = opts && opts.token;
    if (serverUrl == null) { try { serverUrl = await Editor.Profile.getProject('vfx-browser', 'syncUrl'); } catch { /* ignore */ } }
    if (token == null) { try { token = await Editor.Profile.getProject('vfx-browser', 'syncToken'); } catch { /* ignore */ } }
    return {
        serverUrl: (serverUrl || DEFAULT_SYNC_URL).replace(/\/+$/, ''),
        token: token || '',
        mirror: !!(opts && opts.mirror),
    };
}

// A Progress callback that streams sync progress to the (open) browser panel.
function syncProgress(phase: 'up' | 'down') {
    return (msg: string, cur?: number, total?: number) => {
        Editor.Message.broadcast('vfx-browser:sync-progress', {
            phase, msg, cur: cur == null ? null : cur, total: total == null ? null : total,
        });
    };
}

// Cheap file count for the status line — no hashing (buildLocalManifest hashes).
function countLibraryFiles(proj: string): number {
    const root = path.join(proj, LIBRARY_ROOT);
...
export const methods: Record<string, (...args: any[]) => any> = {
    ...
    // Push the project's library-extension up to the sync server (incremental).
    async syncUpLibrary(opts: any) {
        const { serverUrl, token, mirror } = await resolveSyncArgs(opts);
        const proj = projectRoot();
        try {
            const res = await librarySync.syncUp(proj, serverUrl, token, { mirror }, syncProgress('up'));
            Editor.Message.broadcast('vfx-browser:sync-progress', { phase: 'up', done: true, ...res });
            console.log(`[Library] Sync Up: +${res.uploaded} uploaded, ${res.deleted} removed, ${res.unchanged} unchanged -> ${serverUrl}`);
            return { success: true, serverUrl, ...res };
        } catch (e: any) {
            Editor.Message.broadcast('vfx-browser:sync-progress', { phase: 'up', done: true, error: e.message });
            return { success: false, error: e.message };
        }
    },

    // Pull the server library down into the project, then refresh asset-db.
    async syncDownLibrary(opts: any) {
        const { serverUrl, token, mirror } = await resolveSyncArgs(opts);
        const proj = projectRoot();
        try {
            const res = await librarySync.syncDown(proj, serverUrl, token, { mirror }, syncProgress('down'));
            if (res.changed) {
                try { await Editor.Message.request('asset-db', 'refresh-asset', `db://${LIBRARY_ROOT}`); }
                catch (e) { console.warn('[Library] Sync Down: asset-db refresh failed', e); }
            }
            Editor.Message.broadcast('vfx-browser:sync-progress', { phase: 'down', done: true, ...res });
            console.log(`[Library] Sync Down: ${res.downloaded} downloaded, ${res.removed} removed, ${res.unchanged} unchanged <- ${serverUrl}`);
            return { success: true, serverUrl, ...res };
        } catch (e: any) {
            Editor.Message.broadcast('vfx-browser:sync-progress', { phase: 'down', done: true, error: e.message });
            return { success: false, error: e.message };
        }
    },

    // Reachability + counts for the Settings status line (no hashing).
    async getSyncStatus(opts: any) {
        const { serverUrl, token } = await resolveSyncArgs(opts);
        const proj = projectRoot();
        const localCount = countLibraryFiles(proj);
        try {
            const health = await librarySync.ping(serverUrl, token);
            const manifest = await librarySync.fetchManifest(serverUrl, token);
            return { success: true, online: true, serverUrl, version: manifest.version, serverCount: manifest.count, localCount };
        } catch (e: any) {
            return { success: false, online: false, serverUrl, error: e.message, localCount };
        }
    },
...
export async function load() {
    console.log('[Playable Ads Library] Extension loaded');
